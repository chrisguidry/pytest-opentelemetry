...
def test_trace_per_test(pytester: Pytester, span_recorder: SpanRecorder, args: List[str]) -> None:
    pytester.makepyfile(
        """
        from opentelemetry import trace
...
def test_force_flush_with_supported_provider(mock_get_tracer_provider):
    ...
    for plugin in OpenTelemetryPlugin, XdistOpenTelemetryPlugin, PerTestOpenTelemetryPlugin:
